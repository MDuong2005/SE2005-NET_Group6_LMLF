        ...
        <table style="width: 100%; border-collapse: collapse; text-align: left;">
            ...
            <tbody>
                <c:choose>
                    <c:when test="${not empty syllabuses}">
                        <c:forEach var="s" items="${syllabuses}">
                            <tr style="border-bottom: 1px solid #f1f5f9; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#f8fafc'" onmouseout="this.style.backgroundColor='transparent'">
                                ...
                                <td style="padding: 1rem;">${s.currentVersion != null ? s.currentVersion : 'N/A'}</td>
                                <td style="padding: 1rem;">
                                    <c:choose>
                                        <c:when test="${s.status == 'PUBLISHED'}">
                                            <span style="padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; background-color: #bbf7d0; color: #166534;">${s.status}</span>
                                        </c:when>
                                        <c:when test="${s.status == 'DRAFT'}">
                                            <span style="padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; background-color: #fef08a; color: #713f12;">${s.status}</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span style="padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; background-color: #e2e8f0; color: #475569;">${s.status}</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td style="padding: 1rem; text-align: right;">
                                    <a href="${pageContext.request.contextPath}/academic/syllabus?action=detail&id=${s.syllabusId}" style="display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; font-size: 0.75rem; font-weight: 500; color: #1e293b; background-color: white; border: 1px solid #e2e8f0; border-radius: 8px; text-decoration: none; white-space: nowrap; transition: all 0.2s;" onmouseover="this.style.backgroundColor='#f8fafc'" onmouseout="this.style.backgroundColor='white'">
                                        View Detail
                    ...
                    <c:otherwise>
                        <tr>
                            <td colspan="5" style="padding: 2rem; text-align: center; color: #64748b;">No syllabuses found.</td>
                        </tr>
                    </c:otherwise>
                </c:choose>
            </tbody>
