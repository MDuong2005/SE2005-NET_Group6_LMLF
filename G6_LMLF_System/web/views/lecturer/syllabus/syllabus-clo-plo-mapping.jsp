...
<head>
    ...
    <title>Mapping of CLOs to PLOs</title>
    ...
    <style>
        ...
        .clo-plo-matrix th, .clo-plo-matrix td { border: 1px solid #e2e8f0; padding: 10px 12px; text-align: center; font-size: 0.95rem; }
        .clo-plo-matrix .matrix-banner { background: #ea8a44; color: #fff; font-weight: 700; font-size: 1rem; padding: 12px; text-align: center; }
...
<body>
    <header class="syl-header">
        <div class="syl-header-left">
            <a href="${pageContext.request.contextPath}/lecturer/syllabus?action=detail&amp;id=${syllabus.syllabusId}" class="btn-home">
    ...
    <main class="syl-container">
        ...
        <c:choose>
            <c:when test="${not empty syllabusData.curriculumPloGroups and not empty syllabusData.clos}">
                <c:forEach var="group" items="${syllabusData.curriculumPloGroups}">
                    <table class="clo-plo-matrix">
                        <thead>
                            <tr>
                                <th class="matrix-banner" colspan="${group.plos.size() + 1}">
                                    Mapping of CLOs to PLOs of Curriculum <c:out value="${group.curriculumCode}"/>
                                </th>
                            </tr>
                            <tr>
                                <th class="matrix-head clo-col" rowspan="2">CLO</th>
                                <th class="matrix-head" colspan="${group.plos.size()}">PLOs</th>
                            </tr>
                            <tr>
                                <c:forEach var="plo" items="${group.plos}">
                                    <th class="matrix-head"><c:out value="${plo.code}"/></th>
                                </c:forEach>
                            </tr>
                        </thead>
                        <tbody>
                            <c:forEach var="clo" items="${syllabusData.clos}">
                                ...
                                <tr>
                                    ...
                                    <c:forEach var="plo" items="${group.plos}">
                                        <td>
                                            <c:if test="${not empty cloPlos and cloPlos.contains(plo.ploId)}">&#10003;</c:if>
                                        </td>
                                    </c:forEach>
                                </tr>
                            </c:forEach>
            ...
            <c:otherwise>
                <p class="matrix-empty">No CLO-PLO mapping data available for this syllabus.</p>
            </c:otherwise>
        </c:choose>
    </main>
</body>
