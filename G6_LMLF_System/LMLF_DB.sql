-- =======================================================
-- LMLF DATABASE SCHEMA - V5 STANDARD CLEAN
-- Purpose: Academic template -> Designer task -> Designer Excel submit
--          -> Backend auto import -> Reviewer evaluation
-- Note: Use this script for a NEW / EMPTY database.
--       If your database already has data, use the migration file instead.
-- =======================================================
...
-- This table is used for Academic/Admin assigning work to Designer.
-- Reviewer assignment is handled by syllabus_version_review_assignments.
-- =======================================================
...
-- Designer submits Excel into a specific version.
-- =======================================================
CREATE TABLE syllabus_versions (
    ...
    CONSTRAINT chk_sv_status CHECK (status IN ('DRAFT','SUBMITTED','APPROVED','REJECTED','ARCHIVED')),
...
-- =======================================================
-- 12. SYLLABUS VERSION FILES
-- Store original Excel files: Academic template and Designer submission.
-- The actual review display content is imported into syllabus_version_sections.
-- =======================================================
...
-- =======================================================
-- 13. SYLLABUS VERSION SECTIONS
-- Backend imports each Excel sheet into this table.
-- Reviewer Evaluation reads this table by version_id.
-- =======================================================
CREATE TABLE syllabus_version_sections (
    ...
    CONSTRAINT uq_version_section UNIQUE(version_id, section_code),
    CONSTRAINT fk_svs_version FOREIGN KEY (version_id) REFERENCES syllabus_versions(version_id)
);
GO

-- =======================================================
-- 14. REVIEW CRITERIA
-- Active criteria are shown on Reviewer Evaluation page.
-- =======================================================
...
-- =======================================================
-- 15. SYLLABUS VERSION REVIEW ASSIGNMENTS
-- Assign submitted versions to one or more reviewers.
-- =======================================================
...
-- =======================================================
-- 16. SYLLABUS REVIEWS
-- Store final decision of each reviewer for one version.
-- =======================================================
...
-- =======================================================
-- 17. SYLLABUS REVIEW SECTIONS
-- Store decision/comment per review criterion.
-- =======================================================
...
-- Optional structured LO table, separate from imported Excel HTML.
-- =======================================================
...
-- =======================================================
-- 24. REVIEW COMMENTS
-- Threaded comments linked to a review.
-- =======================================================
...
CREATE INDEX idx_syllabuses_course_status ON syllabuses(course_id, status);
CREATE INDEX idx_assignment_designer_status ON syllabus_assignments(designer_id, assignment_status);
CREATE INDEX idx_assignment_course ON syllabus_assignments(course_id, academic_year, semester);
CREATE INDEX idx_syllabus_versions_syl_status ON syllabus_versions(syllabus_id, status);
CREATE INDEX idx_svf_assignment ON syllabus_version_files(assignment_id, file_type);
CREATE INDEX idx_svf_version ON syllabus_version_files(version_id, file_type);
CREATE INDEX idx_svs_version ON syllabus_version_sections(version_id);
CREATE INDEX idx_svra_reviewer_status ON syllabus_version_review_assignments(reviewer_id, status);
CREATE INDEX idx_svra_version ON syllabus_version_review_assignments(version_id);
CREATE INDEX idx_reviews_version ON syllabus_reviews(version_id);
CREATE INDEX idx_srs_review ON syllabus_review_sections(review_id);
...
CREATE INDEX idx_curriculum_pos_cur ON curriculum_pos(curriculum_id);
CREATE INDEX idx_curriculum_plos_cur ON curriculum_plos(curriculum_id);
CREATE INDEX idx_curriculum_plo_po_mappings_po ON curriculum_plo_po_mappings(po_id);
GO
...
-- Only these 5 criteria are displayed on Reviewer Evaluation page.
-- =======================================================
INSERT INTO review_criteria(criteria_code, criteria_name, description, display_order, is_required, is_active)
VALUES
('LEARNING_OUTCOMES', 'Learning Outcomes', 'Review learning outcomes.', 1, 1, 1),
('STUDENT_TASKS', 'Student Tasks', 'Review student tasks.', 2, 1, 1),
('LEARNING_MATERIALS', 'Learning Materials', 'Review learning materials.', 3, 1, 1),
('COURSE_SCHEDULE', 'Course Schedule', 'Review course schedule.', 4, 1, 1),
('COURSE_ASSESSMENT', 'Course Assessment', 'Review assessment plan.', 5, 1, 1);
GO
